...
class sspmod_saml_IdP_SAML2 {
	...
	public static function receiveAuthnRequest(SimpleSAML_IdP $idp) {
		...
		if (isset($_REQUEST['spentityid'])) {
			...
			if (isset($_REQUEST['NameIDFormat'])) {
				...
				$nameIDFormat = $spMetadata->getString('nameID');
				$_SESSION["new_nameID"]=$spMetadata->getString('nameID');
			}
			...
			$forceAuthn = FALSE;
                        ...
                        $forceAuth_centagate = $_SESSION["forceAuth"];
                        if ($forceAuth_centagate === "On"){
                            $forceAuthn=TRUE;
                        }
			...
			SimpleSAML_Logger::info('SAML2.0 - IdP.SSOService: IdP initiated authentication: '. var_export($spEntityId, TRUE));
                        ...
                        //error_log("_________________________SP Method");
			...
			$spMetadata = $metadata->getMetaDataConfig($spEntityId, 'saml20-sp-remote');
                        ...
                        //error_log("________________ FORCE AUTH VALUE FROM CENTAGATE = ".$_SESSION["forceAuth"]);
                        $forceAuth_centagate = $_SESSION["forceAuth"];
                        
                        $forceAuthn=FALSE;
                        if ($forceAuth_centagate === "On"){
                             error_log("______________ FORCE AUTH IS ON ");
                            $forceAuthn=TRUE;
                            $_SESSION["new_nameID"]=$spMetadata->getString('nameID');
                        }else{
                             $_SESSION["new_nameID"]=$spMetadata->getString('nameID');
                             error_log("______________ FORCE AUTH IS OFF , new NAMEID = ".$_SESSION["new_nameID"]);
                        }
		...
		$sessionLostURL = \SimpleSAML\Utils\HTTP::addURLParameters(
			...
			$sessionLostParams);
               ...
               error_log ("new nameID --------> ".$_SESSION["new_nameID"]); 
	...
	private static function generateNameIdValue(SimpleSAML_Configuration $idpMetadata,
		...
		if ($attribute === NULL) {
			...
			if ($attribute === NULL) {
				...
				$attributeValue = $state['Attributes']['userId'][0];
				$idpEntityId = $idpMetadata->getString('entityid');
	...
	private static function buildAssertion(SimpleSAML_Configuration $idpMetadata,
		...
		$a->setNotBefore($now - 30);
                ...
                $sessionTimeout = $state['Attributes']["sessionTimeout"][0];

                error_log("_________________________ SP META  = ".json_encode($state['Attributes']) );

                $sessionNumeric=(int)$sessionTimeout;
                

                if (is_numeric($sessionNumeric) && $sessionNumeric != 0 ){
                       //error_log("______________________ sesstion is numeric ");
                       $sessionLifetime = $sessionTimeout * 60;
                    }else{
                ...
                error_log("______________________ sesstionTimeout =  ".$sessionLifetime);
		...
		if ($spMetadata->getBoolean('simplesaml.attributes', TRUE)) {
                        ...
                        error_log("get metadata atribute_____________");
			...
			$attributes = self::encodeAttributes($idpMetadata, $spMetadata, $state['Attributes']);
		        $attributes['domain']=array($_SERVER['HTTP_HOST']);
		...
		if ($nameIdFormat==NULL && isset($state['Attributes']["nameID"][0])) {
                        if ($state['Attributes']["nameID"][0] === "ID"){
                            $nameIdFormat = SAML2_Const::NAMEID_PERSISTENT;
                        } else {                      
                            $nameIdFormat = SAML2_Const::NAMEID_EMAIL;
                        }
                }
		...
		if (isset($state['saml:NameID'][$nameIdFormat])) {
			...
			if ($spNameQualifier === NULL) {
				$spNameQualifier = $spMetadata->getString('entityid');
                        ...
                        //error_log("______________________ SP NAME ID FORMAT = ".$nameIdFormat);
                        //error_log("______________________ State Attr = ".json_encode($state['Attributes']));

                        $nameID_centagate = $state['Attributes']["nameID"][0];

			//error_log("______________________ NAME ID VALUE FROM CENTAGATE = ".$nameID_centagate);
			...
			if ($nameIdFormat === SAML2_Const::NAMEID_TRANSIENT) {
				...
				if ($nameIdFormat === SAML2_Const::NAMEID_EMAIL) {
					$nameIdValue = $state['Attributes']['email'][0];
				} elseif ($nameIdFormat === SAML2_Const::NAMEID_PERSISTENT) {
					...
					//Fallback to use transient name ID
					$nameIdFormat = SAML2_Const::NAMEID_TRANSIENT;
                                	$nameIdValue = SimpleSAML\Utils\Random::generateID();
				}
			}
		       	
                       if ($nameID_centagate === "ID"){

                            error_log("nameID = ID... ");

                            $nameIdFormat = SAML2_Const::NAMEID_PERSISTENT;
                            //$nameIdValue = $state['Attributes']["userId"][0];
                            $nameIdValue = self::generateNameIdValue($idpMetadata, $spMetadata, $state);
                       }else{

                            error_log("nameID = EMAIL... ");
                      ...
                      $force_auth = $_SESSION["forceAuth"];
                      if (!$force_auth) {
                            $nameID_centagate = $_SESSION["new_nameID"];
                            error_log("new nameID in centagate = ".$nameID_centagate);
                      }

                      //error_log("nameID in centagate = ".$nameID_centagate);
		...
		if ($encryptNameId) {
	       ...
	       error_log("__________________ IDP RESPOND = ".json_encode($nameId));      
	...
	private static function encryptAssertion(SimpleSAML_Configuration $idpMetadata,
		...
		if ($sharedKey !== NULL) {
			...
			//Return unecrypted assertion as the SP does not have an encryption certificate.
			if ($keys === null) {
				return $assertion;
			}
			$key = $keys[0];
